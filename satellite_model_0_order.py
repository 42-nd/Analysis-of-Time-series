...
def Y1(X, tet):
    ret = tet[0] + tet[1] * X + tet[2] * X**2
    for i in range(3, 14, 3):
        ret += tet[i] * np.sin((2 * np.pi * X) / tet[i + 1]) + tet[i + 2] * np.cos(
            (2 * np.pi * X) / tet[i + 1]
        )
...
def Y2(X, tet):
    ...
    # indexes = [4, 7, 10, 13]
    # ret[indexes] = (2 * np.pi) / ret[indexes]
    return ret
...
def loss_f1(tet, X, y):
    return np.sum((Y1(X, tet) - y) ** 2)


def loss_f2(tet, X, y):
    return np.sum((Y2(X, tet) - y) ** 2)
...
tet_pred_parameters_Y0 = sc.optimize.minimize(
    loss_f0, tet_parameters, args=(X, y), method="Powell"
).x
loss_Y0 = loss_f0(tet_pred_parameters_Y0, X, y)
...
# tet_parameters = np.ones(15) * 7
# tet_pred_parameters_Y1 = sc.optimize.minimize(
#     loss_f1, tet_parameters, args=(X, y), method="Powell"
# ).x
# loss_Y1 = loss_f1(tet_pred_parameters_Y1, X, y)
...
# tet_parameters = np.ones(15) * 7
# tet_pred_parameters_Y2 = sc.optimize.minimize(
#     loss_f2, tet_parameters, args=(X, y), method="Powell"
# ).x
# loss_Y2 = loss_f2(tet_pred_parameters_Y2, X, y)

# print(tet_pred_parameters_Y2, loss_Y2, "LOSS Y2")
# plt.plot(X, Y2(X, tet_pred_parameters_Y2), c="black", label="Y2")


plt.plot(X, np.array(df["y"]) * 1e5, label="Original data")
plt.legend()
plt.show()
